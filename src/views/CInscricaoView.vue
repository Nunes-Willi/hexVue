<template>
    <div>
      ...
      <div class="forms-container">
        ...
        <form class="participant-form">
          ...
          <div v-for="participant in participants" :key="participant.id" class="participant-card">
            ...
            <div class="form-row">
              ...
              <input v-model="participant.name" :type="'text'" :id="'participantName_' + participant.id" :name="'participantName_' + participant.id" />
            </div>
            <div class="form-row">
              <label :for="'participantEmail_' + participant.id">E-mail:</label>
              <input v-model="participant.email" :type="'text'" :id="'participantEmail_' + participant.id" :name="'participantEmail_' + participant.id" />
            </div>
          </div>
          <button @click.prevent="addParticipant" class="add-participant-button">Adicionar Participante</button>
        </form>
        ...
        <form class="info-form">
          ...
          <button @click.prevent="showCancelConfirmation" class="cancel-registration-button">Cancelar Inscrição</button>
        </form>
      </div>
      ...
      <div v-if="participants.length > 0" class="participants-list">
        ...
        <div v-for="participant in participants" :key="participant.id" class="participant-card">
          ...
          <button @click="removeParticipant(participant.id)" class="remove-participant-button">Excluir</button>
        </div>
      </div>
  ...
  export default {
    data() {
      return {
        participants: [
          { teamName: "Equipe A", id: 1, name: "Participante 1", email: "email1@example.com" },
          { teamName: "Equipe A", id: 2, name: "Participante 2", email: "email2@example.com" }
        ],
    ...
    methods: {
      addParticipant() {
        this.participants.push({ teamName: this.teamName, id: this.participantIdCounter++, name: "", email: "" });
